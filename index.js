...
const accountBalance = () => {
    ...
    let checkPassword = readline.question('Account found! Insert your password.' + '\n');
    for (let i=0; i < all_users.length; i++) {
        if(all_users[i].id === checkID && all_users[i].password === checkPassword) {
            console.log('Correct password. We validated you as ' + all_users[i].name);
            console.log('Your account balance is ' + all_users[i].balance);
            break;
        } else if (all_users[i].id !== checkID) {
            console.log('An account with that ID does not exist. Try again.');
        } else if (all_users[i].password !== checkPassword) {
